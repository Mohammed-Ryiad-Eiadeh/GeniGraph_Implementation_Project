...
public class AttackDefenceGraph {
    ...
    public List<List<Integer>> initialPopulation(int start, int end, int numOfSolutions) {
        if (start < 1) {
            throw new IllegalStateException("The attackers' entrey node must be positive number starting from 1 S.t: 1, 2, 3, 4, ..., V");
        }
        if (end < 1) {
            throw new IllegalStateException("The sink node or the asset node must be positive number starting from 1 S.t: 1, 2, 3, 4, ..., V");
        }
        if (start > nodes.length) {
            throw new IllegalStateException("The attackers' entrey node must not be a bove the last node we have in our system");
        }
        if (end > nodes.length) {
            throw new IllegalStateException("The sink node or the asset node must not be a bove the last node we have in our system");
        }
        if (numOfSolutions < 1) {
            throw new IllegalStateException("The number of initial solutions must be positive number and preferable higher than 10");
        }
