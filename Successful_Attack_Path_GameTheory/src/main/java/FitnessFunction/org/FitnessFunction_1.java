 ...
 */
public class FitnessFunction_1 {
    private final double[][] adjMatrix;

    /**
     * This constructor is used to initialize an object from the first fitness function which considers the prob of successfull attack
     * @param adjMatrix The adjacency matrix of the given graph
     */
    public FitnessFunction_1(double[][] adjMatrix) {
        if (adjMatrix == null) {
            throw new IllegalArgumentException("The matrix is null!");
        }
        this.adjMatrix = new double[adjMatrix.length][adjMatrix[0].length];
        IntStream.range(0, adjMatrix.length)
