...
public class BehavioralDefender {
    ...
    public BehavioralDefender(double[] edgeWeights, double budget, float alpha) {
        ...
        if (budget <= 0) {
            throw new IllegalArgumentException("The spare bidget must be positive integer");
        }
        if (alpha < 0 || alpha > 1) {
    ...
    public double[] applyBehavioralDefendingForResourceAllocation() {
        ...
        // Probability Weighting Function simulates the Prelec theory
        Arrays.setAll(probabilityWeighting, i -> Math.exp(-Math.pow(-Math.log(Math.exp(-edgeWeights[i])), alpha)));
