 ...
 */
public class GeneticAlgorithm extends FitnessFunction_1 {
     ...
     */
    public GeneticAlgorithm(double[][] adjMat, List<List<Integer>> population, double mutationRate, double crossOverProb, double selectionProportion, int maxIter) {
        super(adjMat);
        this.AdjMat = adjMat;
        if (population == null) {
            ...
            for (int j = 1; j < solution2.size() - 1; j++) {
                if (!solution1.get(i).equals(solution2.get(j))) {
                    if (new Random().nextDouble() > crossOverProb) {
                        ...
                        List<Integer> s1 = new ArrayList<>(super.evaluateSolution(child1) > super.evaluateSolution(solution1)
                                ? child1 : solution1);
                        List<Integer> s2 = new ArrayList<>(super.evaluateSolution(child2) > super.evaluateSolution(solution2)
                                ? child2 : solution2);
                        solution1.clear();
                        solution2.clear();
                        solution1.addAll(s1);
