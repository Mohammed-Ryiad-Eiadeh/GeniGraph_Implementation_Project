...
/**
 * This class is used as the main class for the second-third scenario according to bahavioral defender
 */
public class MainSecondThirdBehavioralDefender {
    public static void main(String[] args) {
        ...
        var assetNode = 13;   // Targetd node by the attacker
        ...
        // This code segment is referred to allocate spare defenders investments (based on importance) on the top-k paths (same time attacks)
        ...
        var sumOfCost = listOfAttackers.stream().mapToDouble(FFun::evaluateSolution).sum();
        ...
        listOfAttackers.forEach(ofAttacker -> IntStream.range(0, ofAttacker.size() - 1).mapToDouble(i -> attackDefenceGraph[ofAttacker.get(i) - 1][ofAttacker.get(i + 1) - 1].totalInvest()).forEach(edgeWeights::add));
        var behavioralDefender = new BehavioralDefender(edgeWeights.stream().mapToDouble(i -> i).toArray(), budget, 0.5f);
        ...
        for (var listOfAttacker : listOfAttackers) {
            for (var i = 0; i < listOfAttacker.size() - 1; i++) {
                ...
                edge.setInvest_D1(edge.getInvest_D1() + edge.addSpareInvestFor_D1(newWeights[i] / 3));
                edge.setInvest_D2(edge.getInvest_D2() + edge.addSpareInvestFor_D2(newWeights[i] / 3));
                edge.setInvest_D3(edge.getInvest_D3() + edge.addSpareInvestFor_D3(newWeights[i] / 3));
