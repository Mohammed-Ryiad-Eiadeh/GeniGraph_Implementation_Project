...
public class GeneticAlgorithm extends FitnessFunction {
    ...
    public GeneticAlgorithm(double[][] adjMat, double[] assetLosses, List<List<Integer>> population, double mutationRate, double crossOverProb, double selectionProportion, int maxIter) {
        ...
        this.AdjMat = new double[adjMat.length][adjMat[0].length];
        System.arraycopy(adjMat, 0, this.AdjMat, 0, adjMat.length);
        this.population = new ArrayList<>();
        this.population.addAll(population);
        if (mutationRate < 0 || mutationRate > 1) {
            throw new IllegalStateException("The mutation rate must be relied in 0 and 1");
        }
        this.mutationRate = mutationRate;
        if (crossOverProb < 0 || crossOverProb > 1) {
            throw new IllegalStateException("The crossover probability must be relied in 0 and 1");
        }
        this.crossOverProb = crossOverProb;
        if (selectionProportion < 0 || selectionProportion > 1) {
            throw new IllegalStateException("The selection probability must be relied in 0 and 1");
        }
        this.selectionProportion = selectionProportion;
        if (maxIter < 0) {
            throw new IllegalStateException("The max iteration must be positive integer");
        }
