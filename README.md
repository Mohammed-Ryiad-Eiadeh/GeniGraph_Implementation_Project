...
# Comparison of GeniGraph and baseline systems on all datasets. The row ``Measurements" show the relative difference $RD$ of the fitness score corresponding to fitness function $F_1$, and the relative difference of the expected cost $CR$ for all defense scenarios. The larger $RD$ and $CR$, the better the defense method with significance level equals 0.05 for the Friedman test.
1) Top-1 attack path.
